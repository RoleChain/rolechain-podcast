...
function getInitialCharacters() {
    ...
    const characterEntries = Object.entries(allCharacters)
        .filter(([name]) => name !== 'Elon Musk' && name !== 'Donald Trump');
...
async function generateLLMResponseWithRetry(character, messages) {
    ...
    while (retryCount < maxRetries) {
        try {
            ...
            const temperature = 0.7; // Increased for more creative responses

            // Add hackathon context to the system message
            const hackathonContext = `
                Important context: There's an exciting AI Agent Hackathon called "Seeds of Agentic Future" happening:
                - Starts February 24th
                - Applications open until March 5th
                - $900,000 in funding available
                - $50,000 in cash prizes
                - Four tracks: Infrastructure & Framework, DeFAI & DeSCI, Gaming & Entertainment, and D.A.T.A
                - Includes mentorship and networking with Web3 leaders
                - Website: agenticfuture.ai
                - Sponsored by: Heurist AI, CARV, Cult World, NGC Ventures, and others
            `;

            // Add hackathon context to messages
